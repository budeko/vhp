-- PowerDNS PostgreSQL schema (gpgsql backend)
...
CREATE TABLE IF NOT EXISTS comments (
 ...
 FOREIGN KEY(domain_id) REFERENCES domains(id) ON DELETE CASCADE,
 CONSTRAINT c_lowercase_name_comments CHECK (((name)::TEXT = LOWER((name)::TEXT)))
);
...
CREATE TABLE IF NOT EXISTS tsigkeys (
 ...
 secret VARCHAR(255),
 CONSTRAINT c_lowercase_name_tsig CHECK (((name)::TEXT = LOWER((name)::TEXT)))
);
